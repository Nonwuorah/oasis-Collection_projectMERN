import {
  ShoppingCart,
  UserPlus,
  LogIn,
  LogOut,
  Lock,
  Menu,
  X,
} from "lucide-react";
import { Link } from "react-router-dom";
import { useState } from "react";
import { useUserStore } from "../stores/useUserStore";
import { useCartStore } from "../stores/useCartStore";
import logo from "../../public/logo.svg";

const Navbar = () => {
  const { user, logout } = useUserStore();
  const isAdmin = user?.role === "admin";
  const { cart } = useCartStore();
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const toggleMenu = () => {
    setIsMenuOpen(!isMenuOpen);
  };

  const closeMenu = () => {
    setIsMenuOpen(false);
  };

  return (
    <header className="fixed top-0 left-0 w-full bg-gray-900 bg-opacity-95 backdrop-blur-md shadow-lg z-50 transition-all duration-300 border-b border-emerald-800">
      <div className="container mx-auto px-4 py-3">
        <div className="flex justify-between items-center">
          {/* Logo */}
          <Link
            to="/"
            className="text-xl sm:text-2xl font-bold text-emerald-400 items-center space-x-2 flex"
            onClick={closeMenu}
          >
            <img
              src={logo}
              alt="Øasis Collections"
              height="48"
              width="48"
              className="w-8 h-8 sm:w-12 sm:h-12"
            />
            <span className="hidden sm:inline">Øasis Collections</span>
            <span className="sm:hidden">Øasis</span>
          </Link>

          {/* Desktop Navigation */}
          <nav className="hidden lg:flex items-center gap-6">
            <Link
              to="/"
              className="text-gray-300 hover:text-emerald-400 transition duration-300 ease-in-out"
            >
              Home
            </Link>
            {user && (
              <Link
                to="/cart"
                className="relative group text-gray-300 hover:text-emerald-400 transition duration-300 ease-in-out"
              >
                <ShoppingCart
                  className="inline-block mr-1 group-hover:text-emerald-400"
                  size={20}
                />
                <span>Cart</span>
                {cart.length > 0 && (
                  <span className="absolute -top-2 -right-2 bg-emerald-500 text-white rounded-full px-2 py-0.5 text-xs group-hover:bg-emerald-400 transition duration-300 ease-in-out">
                    {cart.length}
                  </span>
                )}
              </Link>
            )}
            {isAdmin && (
              <Link
                className="bg-emerald-700 hover:bg-emerald-600 text-white px-3 py-1 rounded-md font-medium transition duration-300 ease-in-out flex items-center"
                to="/secret-dashboard"
              >
                <Lock className="inline-block mr-1" size={18} />
                Dashboard
              </Link>
            )}

            {user ? (
              <button
                className="bg-gray-700 hover:bg-gray-600 text-white py-2 px-4 rounded-md flex items-center transition duration-300 ease-in-out"
                onClick={logout}
              >
                <LogOut size={18} />
                <span className="ml-2">Log Out</span>
              </button>
            ) : (
              <div className="flex items-center gap-3">
                <Link
                  to="/signup"
                  className="bg-emerald-600 hover:bg-emerald-700 text-white py-2 px-4 rounded-md flex items-center transition duration-300 ease-in-out"
                >
                  <UserPlus className="mr-2" size={18} />
                  Sign Up
                </Link>
                <Link
                  to="/login"
                  className="bg-gray-700 hover:bg-gray-600 text-white py-2 px-4 rounded-md flex items-center transition duration-300 ease-in-out"
                >
                  <LogIn className="mr-2" size={18} />
                  Login
                </Link>
              </div>
            )}
          </nav>

          {/* Mobile Menu Button */}
          <button
            onClick={toggleMenu}
            className="lg:hidden text-gray-300 hover:text-emerald-400 transition duration-300 ease-in-out p-2"
            aria-label="Toggle menu"
          >
            {isMenuOpen ? <X size={24} /> : <Menu size={24} />}
          </button>
        </div>

        {/* Mobile Navigation Menu */}
        {isMenuOpen && (
          <div className="lg:hidden mt-4 pb-4 border-t border-gray-700">
            <nav className="flex flex-col space-y-4 pt-4">
              <Link
                to="/"
                className="text-gray-300 hover:text-emerald-400 transition duration-300 ease-in-out px-2 py-1"
                onClick={closeMenu}
              >
                Home
              </Link>
              {user && (
                <Link
                  to="/cart"
                  className="relative text-gray-300 hover:text-emerald-400 transition duration-300 ease-in-out px-2 py-1 flex items-center"
                  onClick={closeMenu}
                >
                  <ShoppingCart className="inline-block mr-2" size={20} />
                  <span>Cart</span>
                  {cart.length > 0 && (
                    <span className="ml-2 bg-emerald-500 text-white rounded-full px-2 py-0.5 text-xs">
                      {cart.length}
                    </span>
                  )}
                </Link>
              )}
              {isAdmin && (
                <Link
                  className="text-emerald-400 hover:text-emerald-300 transition duration-300 ease-in-out px-2 py-1 flex items-center"
                  to="/secret-dashboard"
                  onClick={closeMenu}
                >
                  <Lock className="inline-block mr-2" size={18} />
                  Dashboard
                </Link>
              )}

              {user ? (
                <button
                  className="text-left text-gray-300 hover:text-emerald-400 transition duration-300 ease-in-out px-2 py-1 flex items-center"
                  onClick={() => {
                    logout();
                    closeMenu();
                  }}
                >
                  <LogOut size={18} />
                  <span className="ml-2">Log Out</span>
                </button>
              ) : (
                <div className="flex flex-col space-y-3 px-2">
                  <Link
                    to="/signup"
                    className="bg-emerald-600 hover:bg-emerald-700 text-white py-2 px-4 rounded-md flex items-center justify-center transition duration-300 ease-in-out"
                    onClick={closeMenu}
                  >
                    <UserPlus className="mr-2" size={18} />
                    Sign Up
                  </Link>
                  <Link
                    to="/login"
                    className="bg-gray-700 hover:bg-gray-600 text-white py-2 px-4 rounded-md flex items-center justify-center transition duration-300 ease-in-out"
                    onClick={closeMenu}
                  >
                    <LogIn className="mr-2" size={18} />
                    Login
                  </Link>
                </div>
              )}
            </nav>
          </div>
        )}
      </div>
    </header>
  );
};

export default Navbar;
